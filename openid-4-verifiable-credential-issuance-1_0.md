...
* `c_nonce`: OPTIONAL. String containing a nonce for key proof possession (see #credential-request). The Wallet MUST use this nonce for subsequent requests until a new nonce is issued by the Credential Issuer. Each issued `c_nonce` value MUST be unique, a Credential Issuer cannot issue duplicate `c_nonces` values.
