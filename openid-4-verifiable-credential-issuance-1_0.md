...
* Grant Type `authorization_code`:
  ...
  * `authotization_server`: OPTIONAL when a Credential Issuer is relying on multiple Authorization Servers. MUST NOT be used otherwise. String that the Wallet can use to identify the Authorization Server to used with this grant type when `authoziation_servers` parameter in the Credential Issuer metadata has multiple entries. MUST match with one of the values in the `authoziation_servers` array obtained from the Credential Issuer metadata.
