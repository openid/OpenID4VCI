...
* `jwt`: objects of this type contain a single `jwt` element with a JWS [@!RFC7515] as proof of possession. The JWT MUST contain the following elements:
    * `kid`: CONDITIONAL. As specified by [@!RFC7515]. This claim MUST NOT be present if either of the `jwk` or `x5c` claims are present.
    * `jwk`: CONDITIONAL. As specified by [@!RFC7515]. This claim MUST NOT be present if either the `kid` or `x5c` claims are present.
    * `x5c`: OPTIONAL. As specified by [@!RFC7515]. This claim MUST NOT be present if either of the `kid` or `jwk` claims are present.
    * `iss`: REQUIRED. As specified by [@!RFC7519], the value of this claim MUST correspond to the client_id of the client making the credential request.
    * `aud`: REQUIRED. As specified by [@!RFC7519], the value of this claim MUST correspond to the issuer URL of credential issuer.
    * `iat`: REQUIRED. As specified by [@!RFC7519], the value of this claim MUST correspond to the time at which the proof was created.
    * `nonce`: REQUIRED. The value type of this claim MUST be a string, where the value is a nonce provided by the credential issuer.

The `proof` element MUST incorporate a fresh nonce value generated by the Credential issuer and the Credential issuer's identifier (audience) to allow the Credential issuer to detect replay. The way that data is incorporated depends on the proof type. In a JWT, for example, the nonce is conveyed in the `nonce` claims whereas the audience is conveyed in the `aud` claim. In a Linked Data proof, for example, the nonce is included as the `challenge` element in the proof object and the issuer (the intended audience) is included as the `domain` element.
