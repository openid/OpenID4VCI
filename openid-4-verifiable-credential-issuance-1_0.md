...
* `batch_credential_issuance`: OPTIONAL. Object containing information about the Credential Issuer's supports for batch issuance of Credentials on the Credential Endpoint. The presence of this parameter means that the issuer can issue more than one Verifiable Credential for the same Credential Dataset on the same request.
...
* Issue a batch of Credentials with the same Credential Dataset to enable the usage of a unique Credential per presentation or per Verifier. This only helps with Verifier/Verifier unlinkability.
