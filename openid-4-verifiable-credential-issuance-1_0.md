...
* `proof`: OPTIONAL. JSON object containing proof of possession of the cryptographic key material the issued Credential would be bound to.  The `proof` object MUST contain a following claim:
...
* `c_instance_identifier`: REQUIRED when `c_instance_identifier` was returned from the Token Response. JSON string that identifies a Credential that is being requested to be issued. When this parameter is used, the `format` parameter and any other Credential format specific set of parameters such as those defined in (#format_profiles) MUST NOT be present.
...
* `c_instance_identifiers_supported`: OPTIONAL. Boolean value specifying whether the Credential Issuer supports returning `c_instance_identifiers` parameter in the `authorization_details` Token Response parameter, with `true` indicating support. If omitted, the default value is `false`.
