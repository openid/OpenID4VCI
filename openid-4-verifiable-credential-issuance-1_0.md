...
* `interval`: OPTIONAL. Contains a number that represents the minimum amount of time in seconds that the Wallet needs to wait before providing a new request to the Deferred Credential Endpoint. It MUST NOT be used if the `credentials` parameter is present. If not present, the Wallet MUST use `86400` as the default value.
