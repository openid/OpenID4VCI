...
Note: Claims in the Credential are about the subject of the Credential, which is often End-User who possesses it.
...
The `proof_type` is an extension point that enables the use of different types of proofs for different cryptographic schemes.
...
The `format` value determines the Credential format and encoding of the credential in the Credential Response. Details are defined in the Credential Format Profiles in (#format_profiles).
