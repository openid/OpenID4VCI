...
* `jwt`: objects of this type contain a single `jwt` element with a JWS [@!RFC7515] as proof of possession. The JWT MUST contain the following elements:
  * in the JWT header,
    * `typ`: REQUIRED. MUST be `vciproof+jwt`, which explicitly types the proof JWT as recommended in Section 3.11 of [@!RFC8725].
    * `alg`: REQUIRED. A digital signature algorithm identifier such as per [@!RFC7518]. MUST NOT be none or an identifier for a symmetric algorithm (MAC).
