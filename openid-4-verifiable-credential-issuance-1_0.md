...
* `jwt`: objects of this type contain a single `jwt` element with a JWS [@!RFC7515] as proof of possession. The JWT MUST contain the following elements:
    ...
    * `iat`: REQUIRED (number). The value of this claim MUST be the time at which the proof was created. Its value is a JSON number representing the number of seconds from 1970-01-01T0:0:0Z as measured in UTC until the date/time
...
The `proof` element MUST incorporate a nonce value generated by the Credential issuer and the Credential issuer's identifier (audience) to allow the Credential issuer to detect replay.
