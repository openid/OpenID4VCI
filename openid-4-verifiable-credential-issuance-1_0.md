...
* `error`: REQUIRED. The `error` parameter SHOULD be a single ASCII [@!USASCII] error code from the following:
  ...
  * `invalid_proof`: The `proof` or `proofs` parameter in the Credential Request is invalid. For example, this could be because either both fields are missing, both are present simultaneously, one of the provided key proofs is either invalid or if a `c_nonce` was provided by the Credential Issuer, one of the key proofs is not linked to a previously issued valid `c_nonce` value.
