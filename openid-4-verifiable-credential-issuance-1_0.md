...
Credential Issuer:
:  Entity that issues Verifiable Credentials. Also called Issuer. In the context of this specification, the Credential Issuer acts as an OAuth 2.0 Authorization Server (see [@!RFC6749]).
...
Every Credential Issuer utilizes an OAuth [@!RFC6749] authorization server to authorize access. The same OAuth authorization server can protect one or more Credential Issuers. Wallets determine the authorization server a certain Credential Issuer relies on using the Credential Issuer's metadata.   
...
The Wallet MAY send one Batch Credential Request to the Batch Credential Endpoint to request 
...
The Credential Issuer sends Credential Offer as a HTTP GET request or a HTTP redirect to the Credential Offer Endpoint URL defined in (#client-metadata).
...
The Credential Issuer MAY render a QR code containing the Credential Offer that can be scanned by the user using a Wallet, or a deeplink that the user can click.
...
Below is a non-normative example of an Credential Offer passed by value:
...
Response from the Credential Issuer that contains an Credential Offer Object MUST have the media type "application/json".
...
* `missing_proof`: Credential Request did not contain a `proof`, and one is
  required in order to issue the requested credential
* `invalid_proof`: The `proof` provided was invalid, e.g., it was not bound to a
  nonce provided by the Credential Issuer
...
* `format`: REQUIRED. A JSON string identifying the format of this credential, e.g. `jwt_vc_json` or `ldp_vc`. Depending on the format value, the Supported Credentials Object contains further elements defining the type and (optionally) particular claims the credential may contain, and information how to display the credential. (#format_profiles) defines Credential Format Profiles introduced by this specification. 
