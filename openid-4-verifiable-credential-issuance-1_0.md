...
: A persistent identifier that refers to a specific version of a Credential Dataset. It remains stable across multiple instances of a Credential that share the same set of claims, even if they differ in cryptographic proofs. When the claims in the dataset change, a new Credential Dataset Identifier is assigned. This enables clients to detect meaningful changes to the underlying data and to distinguish between different versions of a Credential Dataset issued under the same Credential Configuration.
