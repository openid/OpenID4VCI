...
* Grant Type `authorization_code`:
  ...
  * `authorization_servers`: OPTIONAL when a Credential Issuer is relying on multiple Authorization Servers. MUST NOT be used otherwise. String that the Wallet can use to identify the Authorization Server to used with this grant type when `authorization_servers` parameter in the Credential Issuer metadata has multiple entries. MUST match with one of the values in the `authorization_servers` array obtained from the Credential Issuer metadata.
* Grant Type `urn:ietf:params:oauth:grant-type:pre-authorized_code`:
  ...
  * `authorization_server`: OPTIONAL when a Credential Issuer is relying on multiple Authorization Servers. MUST NOT be used otherwise. String that the Wallet can use to identify the Authorization Server to used with this grant type when `authoziation_servers` parameter in the Credential Issuer metadata has multiple entries. MUST match with one of the values in the `authorization_servers` array obtained from the Credential Issuer metadata.
...
Note: When using relying on multiple Authorization Servers for authorization, Credential Issuer MUST be able to identify from which Authorization Server each Access Tokens is coming.
