...
The Deferred Credential Response MAY use the `notification_id` and `credential_metadata` parameters as defined in (#credential-response). If it uses the `credentials` parameter, it SHOULD also use the `credential_dataset_id` and `credential_dataset_tag` parameters as defined in (#credential-response).
